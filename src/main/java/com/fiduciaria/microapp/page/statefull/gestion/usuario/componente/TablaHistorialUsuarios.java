...
public class TablaHistorialUsuarios extends BasePanel {
  @SpringBean
  ...
  public TablaHistorialUsuarios(String id) {
    this(id, (IModel<?>)null);
  }
  
  public TablaHistorialUsuarios(String id, IModel<?> model) {
    super(id, model);
    boolean isuauriouca = AuthenticatedWebSession.get().getRoles().hasRole("UCA");
    this.tracedb = new TraceVM(this.httgwy, isuauriouca, usuarioSesion());
  ...
  private void initDataTable() {
    initColumns();
    AjaxFallbackSimpleDataTable tablaHistorial = new AjaxFallbackSimpleDataTable("table", this.columns, (ISortableDataProvider)new SortableHistorialUsuarioDataProvider(this.tracedb), 10) {
    ...
    tablaHistorial.addBottomToolbar((AbstractToolbar)(new ExportToolbar((DataTable)tablaHistorial, (IModel)Model.of("Exportar Historial Usuarios a "), nombreArchivoExportado)).addDataExporter((IDataExporter)new CSVDataExporter() {
            protected IConverterLocator getConverterLocator() {
              return (IConverterLocator)TablaHistorialUsuarios.this;
            }
            
            protected <T> IModel<T> wrapModel(IModel<T> model) {
              return TablaHistorialUsuarios.this.wrap(model);
            }
          }));
  }
  ...
  private Component initSeccionFiltros() {
    FilterRastroToolbar opcionesFiltroBar = new FilterRastroToolbar("filterSeccion", (IModel)new Model((Serializable)this.tracedb)) {
        public void onEventoActualizar(AjaxRequestTarget target) {
          ...
          TablaHistorialUsuarios.this.initColumns();
        }
      };
